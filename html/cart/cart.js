...
let loadCart = async () => {
  ...
  const cartWithPriceP = cart.map(async (product) => {
    const apiProduct = await fetch(baseUrl + product.id).then((response) => {
  ...
  cart.forEach((item) => {
    items.insertAdjacentHTML(
      ...
      `
            ...
            <div class="cart__item__img">
               ...
               <img src="${item.imageUrl}" alt="${item.altTxt}">
                ...
                <div class="cart__item__content">
                    ...
                    <div class="cart__item__content__settings__quantity" id="quantity${
                      item.id + item.color
                    }">
                        <p>Qté :  </p>
                        <input type="number" class="itemQuantity" name="itemQuantity" min="1" max="100" id="${
                          item.id + item.color
                        }" value="${item.quantity}" data-id="${
        item.id
      }" data-color="${
        item.color
      }" onchange="updateQauntityItem(dataset.id, dataset.color,this.value)">
                    </div>
        ...
        item.color
      }" onclick="deletItem(dataset.id, dataset.color)">Supprimer</p>
                    </div>
                    </div>
                </div>
            </article>`
...
let updateQauntityItem = (id, color, quantity) => {
  ...
  if (q <= 0 || q > 100) {
    document
      .querySelector("#quantity" + id + color)
      .insertAdjacentHTML(
        "beforebegin",
        alertColorInvalid("Veuillez choisir une valeur comprise entre 1 et 100")
      );
...
let sendOrder = () => {
  ...
  if (!products.length > 0) {
    document
      ...
      .insertAdjacentHTML(
        "beforebegin",
        alertColorInvalid("Votre cart est vide")
      );
    setTimeout(() => removealert(), 3000);
    return 0;
  }
  ...
  if (!address.length > 0) {
    document.getElementById("addressErrorMsg").innerText =
      "le champ adresse ne doit pas être null";
    return 0;
  } else {
    document.getElementById("addressErrorMsg").innerText = "";
  }
  ...
  fetch("http://localhost:3000/api/products/order", {
    ...
    .then((orderDetail) => {
      ...
      window.location.replace(
        "../confirmation/confirmation.html?orderId=" + orderDetail.orderId
      );
    })
    .catch((error) => {
      document
        ...
        .insertAdjacentHTML(
          "beforeend",
          `<p id="submitErrorMsg"> "Une erreur s'est produite, réessayez plus tard" </p>`
        );
    })
    .finally(() => {
